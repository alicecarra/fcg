...
GLuint BuildTriangles()
{
    ...
    GLfloat NDC_coefficients[4 * 17] = {0};
    NDC_coefficients[3] = 1.0f;
    for (int i = 1; i <= segments; ++i, angle += step)
    {
        NDC_coefficients[4 * i + 0] = cos(angle) * radius;
        NDC_coefficients[4 * i + 1] = sin(angle) * radius;
        NDC_coefficients[4 * i + 2] = 0.0f;
        NDC_coefficients[4 * i + 3] = 1.0f;
    }
    ...
    // prettier-ignore
    GLfloat color_coefficients[17 * 4] = {0};
    ...
    //
    GLubyte indices[18] = {0}; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
    for (int i = 0; i <= 16; i++)
    {
        indices[i] = i;
    }
